...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	for i, entry := range workout.Entries {
		...
		RETURNING id
		`
		err = tx.QueryRow(query, workout.ID, entry.ExerciseName, entry.Sets, entry.Reps, entry.DurationSeconds, entry.Weight, entry.Notes, i).Scan(&entry.ID)
		if err != nil {
			return nil, err
		}
	}
...
func (pg *PostgresWorkoutStore) GetWorkoutByID(id int64) (*Workout, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(
			&entry.ID, &entry.ExerciseName, &entry.Sets,
			&entry.Reps, &entry.DurationSeconds,
